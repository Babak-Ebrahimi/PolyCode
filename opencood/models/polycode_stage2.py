...
from opencood.models.polycode_stage1 import CrossAttention, detect_head
...
class PolyCodeStage2(nn.Module):
    def __init__(self, args) -> None:
        super(PolyCodeStage2, self).__init__()
        ...
        if self.sensor_type_k == "lidar":
            ...
            self.encoder_k = train_utils.load_pretrained_submodule(    
                args["encoder_k"]["saved_pth"], self.encoder_k, "encoder_m4" 
            )
    ...
    def forward(self, data_dict):
        ...
        for cav_num in record_len.tolist():
            ...
            if neb_num == 0:
                neb_num = 1
            neb_num_sum += neb_num
        data_dict["inputs_neb"]['neb_sum'] = neb_num_sum 
        ...
        #self.visualization(key[0], save_path='./key1_beforecodebook_neighbor_sd1.png', pooling='max')
        key_loss_k = zero
        # dimension alignment
        key = self.compressor_k(key)     # [B, Ck, H, W] e.g., [B,256,64,128]
        B, C, H, W = key.shape
        # UMGM expects [N, C]; flatten over spatial on the FINAL size
        key_2d = key.permute(0, 2, 3, 1).contiguous().view(-1, C)    # [B*H*W, C]
        key_2d, _, _, codebook_loss_k = self.codebook_k(key_2d)
        key = key_2d.view(B, H, W, C).permute(0, 3, 1, 2).contiguous()  # back to [B,C,H,W]
        #self.visualization(key[0], save_path='./key2_aftercodebook_neighbor_sd1.png', pooling='max')
        ...
        # expose codebook loss for the criterion
        output_dict['codebook_loss_k'] = codebook_loss_k
        output_dict['codebook_loss_total']=output_dict['codebook_loss_k']
        # optional: if your criterion doesn’t aggregate it, you can pre-sum here:
        # w = self.loss_weights.get("umgm_codebook", 1.0)
        # output_dict['loss'] = output_dict.get('loss', query.new_zeros(())) + w * codebook_loss_k
