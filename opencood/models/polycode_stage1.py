...
class PolyCodeStage1(nn.Module):
    def __init__(self, args) -> None:
        super(PolyCodeStage1, self).__init__()
       ...
       ###################################### compressor K 
        ...
        channel_k_in = int(args["encoder_k"]["args"]["channel"]) # 64
        channel_k_out = args["compressor_k"]['channel']

        p_rate = 0.0
        seg_num = args['compressor_k']['seg_num']
        dict_size = [args['compressor_k']['dict_size'], args['compressor_k']['dict_size'], args['compressor_k']['dict_size']]

        self.codebook_k = UMGMQuantizer(
                        channel_k_out, seg_num, dict_size, p_rate,
                        {
                            "latentStageEncoder":   lambda: nn.Linear(channel_k_out, channel_k_out),
                            "quantizationHead":     lambda: nn.Linear(channel_k_out, channel_k_out),
                            "latentHead":           lambda: nn.Linear(channel_k_out, channel_k_out), 
                            "restoreHead":          lambda: nn.Linear(channel_k_out, channel_k_out),
                            "dequantizationHead":   lambda: nn.Linear(channel_k_out, channel_k_out), 
                            "sideHead":             lambda: nn.Linear(channel_k_out, channel_k_out),
                        }
                    )
        # if compressor_k_model_path:
        #     sd = torch.load(compressor_k_model_path, map_location="cpu")
        #     self.compressor_k.load_state_dict(sd, strict=False)

        #######################################compressor v


        channel_v_in = int(args["encoder_v"]["args"]["channel"]) # 64
        channel_v_out = args["compressor_v"]['channel']

        p_rate = 0.0
        seg_num = args['compressor_v']['seg_num']
        dict_size = [args['compressor_v']['dict_size'], args['compressor_v']['dict_size'], args['compressor_v']['dict_size']]

        self.codebook_v = UMGMQuantizer(
                        channel_v_out, seg_num, dict_size, p_rate,
                        {
                            "latentStageEncoder":   lambda: nn.Linear(channel_v_out, channel_v_out),
                            "quantizationHead":     lambda: nn.Linear(channel_v_out, channel_v_out),
                            "latentHead":           lambda: nn.Linear(channel_v_out, channel_v_out), 
                            "restoreHead":          lambda: nn.Linear(channel_v_out, channel_v_out),
                            "dequantizationHead":   lambda: nn.Linear(channel_v_out, channel_v_out), 
                            "sideHead":             lambda: nn.Linear(channel_v_out, channel_v_out),
                        }
                    )
        # if compressor_v_model_path:
        #     sd_v = torch.load(compressor_v_model_path, map_location="cpu")
        #     self.compressor_v.load_state_dict(sd_v, strict=False)

        self.specific_prompt_k_temp = torch.zeros(channel_k_in, h_q, w_q)
        self.specific_prompt_k = nn.Parameter(self.specific_prompt_k_temp, requires_grad=True)
        

        
        self.specific_prompt_v_temp = torch.zeros(channel_v_in, h_q, w_q)
    ...
    def proj_neb2ego(self, neb_feat: torch.Tensor, 
        ...
        Args:
            ...
            ego2neb_t_matrix:   [1, max_cav, 2, 3] 
        Returns:
            projected_neb_feat: neb feature.
        """
    ...
    def forward(self, data_dict):
        ...
        if data_dict['selected_char'] == 'k':
            ...
            B, C, H, W = key.shape
            key_nhwc,_,_,key_loss_k= self.codebook_k(key.permute(0, 2, 3, 1).contiguous().view(-1,C)) 
            key = key_nhwc.view(B, H, W, C).permute(0, 3, 1, 2).contiguous()
            ...
            key=self.compressor_v(key)
            B, C, H, W = key.shape
            key_nhwc,_,_,key_loss_v = self.codebook_v(key.permute(0, 2, 3, 1).contiguous().view(-1,C))
            key = key_nhwc.view(B, H, W, C).permute(0, 3, 1, 2).contiguous()
        ...
        # output_dict['loss_dict'].update({
        #                                 'codebook_loss_k': key_loss_k,
        #                                 'codebook_loss_v': key_loss_v,
        #                                 'codebook_loss_total': key_loss_k + key_loss_v,
        #                                 })
        output_dict['codebook_loss_k']=key_loss_k 
        output_dict['codebook_loss_v']=key_loss_v 
        output_dict['codebook_loss_total']=output_dict['codebook_loss_k']+output_dict['codebook_loss_v']
