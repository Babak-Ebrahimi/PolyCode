...
class PointPillarPyramidLoss(PointPillarDepthLoss):
    ...
    def calc_occ_loss(self, occ_single_list, positives, negatives, batch_size):
        ...
        # feature_show(torch.cat([occ_positives.permute(0,3,1,2)[0], \
        #             occ_negatives.permute(0,3,1,2)[0]], dim=0), 'analysis/all')
        # feature_show(occ_positives.permute(0,3,1,2)[0], 'analysis/positive')
        # feature_show(occ_negatives.permute(0,3,1,2)[0], 'analysis/negative')
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix="", pbar=None) :
        ...
        print_msg = "[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"" || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || Depth Loss: %.4f || Pyramid Loss: %.4f" \
                 % (
