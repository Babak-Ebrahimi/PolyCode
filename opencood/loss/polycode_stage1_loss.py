...
class PolyCodeStage1Loss(nn.Module):
    def __init__(self, args):
        super(PolyCodeStage1Loss, self).__init__()
        ...
        self.weight_codebook=args['codebook']['weight']
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        batch_size = output_dict['record_len'].size(0)
        ...
        key_lens = output_dict['record_len']-1
        key_lens[key_lens==0] += 1 

        cls_labls_key = torch.repeat_interleave(cls_labls, key_lens, dim=0)
        positives_key = torch.repeat_interleave(positives, key_lens, dim=0)
        negatives_key = torch.repeat_interleave(negatives, key_lens, dim=0)
        pos_normalizer_key = positives_key.sum(1, keepdim=True).float()
        batch_size_key = output_dict[f'psm_key'].size(0)

        # 计算cls loss key
        cls_preds = output_dict[f'psm_key'].permute(0, 2, 3, 1).contiguous() \
                    .view(batch_size_key, -1,  1)
        cls_weights = positives_key * self.pos_cls_weight + negatives_key * 1.0
        cls_weights /= torch.clamp(pos_normalizer_key, min=1.0)
        cls_loss_key = sigmoid_focal_loss(cls_preds, cls_labls_key, weights=cls_weights, **self.cls)
        cls_loss_key = cls_loss_key.sum() * self.cls['weight'] / batch_size_key

        # 计算reg loss key
        reg_weights = positives_key / torch.clamp(pos_normalizer_key, min=1.0)
        reg_preds = output_dict[f'rm_key'].permute(0, 2, 3, 1).contiguous().view(batch_size_key, -1, 7)
        reg_targets = target_dict['targets'].view(batch_size, -1, 7)
        reg_targets_key = torch.repeat_interleave(reg_targets, key_lens, dim=0)
        reg_preds, reg_targets_key = self.add_sin_difference(reg_preds, reg_targets_key)
        reg_loss_key = weighted_smooth_l1_loss(reg_preds, reg_targets_key, weights=reg_weights, sigma=self.reg['sigma'])
        reg_loss_key = reg_loss_key.sum() * self.reg['weight'] / batch_size_key
        ...
        if output_dict['selected_char'] == 'k':
            target_k = torch.zeros(out_k.size(0), dtype=torch.long, device=out_k.device)
        elif output_dict['selected_char'] == 'v':
            target_k = torch.zeros(out_k.size(0), dtype=torch.long, device=out_k.device)+2
        ...
        codebook_loss_k=output_dict['codebook_loss_k']
        codebook_loss_v=output_dict['codebook_loss_v']

        codebook_loss=output_dict['codebook_loss_total']*self.weight_codebook

        total_loss += reg_loss + cls_loss + reg_loss_key + cls_loss_key + codebook_loss

        self.loss_dict.update({'total_loss': total_loss.item(),
                               'reg_loss': reg_loss.item(),
                               'cls_loss': cls_loss.item(),
                               'reg_loss_key': reg_loss_key.item(),
                               'cls_loss_key': cls_loss_key.item(),
                               'specific_loss': specific_loss.item(), 
                               'style_loss': style_loss.item(),
                               'codebook_loss_k': codebook_loss_k.item(),
                               'codebook_loss_v': codebook_loss_v.item(),
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, pbar = None, suffix=""):
        ...
        codebook_k_loss=self.loss_dict.get('codebook_loss_k', 0)
        codebook_v_loss=self.loss_dict.get('codebook_loss_v', 0)
        ...
        if not writer is None:
            ...
            writer.add_scalar('Codebook_loss_k'+suffix, codebook_k_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Codebook_loss_v'+suffix, codebook_v_loss,
                            epoch*batch_len + batch_id)
